...
header("Content-Type: text/html;charset=utf-8");
  ...
  function add_category(){
    ...
    $rows=xiu_execute("insert into categories value (null,'{$slug}','{$name}');");
...
<?php  include 'inc/siderbar.php'; ?>
  ...
  $(function($){
    ...
    $tbodyCheckboxs.on('change',function(){
      ...
      var id=$(this).data('id');
      // 根据有没有选中当前这个CheckBox决定是添加还是删除
      if($(this).prop(('checked'))){
        // allCheckeds.push(id)===-1||allCheckeds.push(id);
        // allCheckeds.includes(id)判断数组中是否存在当前ID
        allCheckeds.includes(id)||allCheckeds.push(id);
      }else{
        allCheckeds.splice(allCheckeds.indexOf(id),1);
      }
      //根据剩下多少选中的CheckBox决定是否显示批量删除按钮
    ...
    $('thead input').on('change',function(){
      ...
      //attr只有第一有用，第二次则失效
      // $tbodyCheckboxs.attr('checked',checked);
    });
    ...
    /*$tbodyCheckboxs.on('change',function(){
     ...
     $tbodyCheckboxs.each(function(i,item){
      ...
      flag=true ?btnDelete.fadeIn() : btnDelete.fadeOut(); 
